        ...
        double val[], MM_typecode matcode)
{
    ...
    {
        for (i=0; i<nz; i++)
            if (fscanf(f, "%d %d %lg %lg", &ia[i], &ja[i], &val[2*i], &val[2*i+1])
...
int mm_read_mtx_crd_entry(FILE *f, int *ia, int *ja,
        double *real, double *imag, MM_typecode matcode)
{
    if (mm_is_complex(matcode))
    {
            if (fscanf(f, "%d %d %lg %lg", ia, ja, real, imag)
                != 4) return MM_PREMATURE_EOF;
    }
    else if (mm_is_real(matcode))
    {
    ...
    *val = NULL;
    ...
    if (mm_is_complex(*matcode))
    {
        *val = (double *) malloc(*nz * 2 * sizeof(double));
        ret_code = mm_read_mtx_crd_data(f, *M, *N, *nz, *ia, *ja, *val,
                *matcode);
